...
export const LOCATIONS: LocationPage[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      // NOTE: the source doc's copy for this page ends here — distance/drive-time was left
      // as an unfilled "xx kms and xx minutes drive" placeholder, and no further unique
      // paragraphs were written for this locality. The shared celebration-spaces summary
      // below is reused (as the sibling pages themselves reuse it, reworded each time) so
      // the page isn't left empty, but this page still needs bespoke copy from the client.
...
export const LOCALITY_SLUGS = LOCATIONS.filter((l) => !l.isPillar).map((l) => l.slug)
