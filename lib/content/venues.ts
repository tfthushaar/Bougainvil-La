...
type Manifest = Record<string, { 'with-decor': string[]; 'without-decor': string[] }>

export const VENUES: Venue[] = VENUE_SEEDS.map((v) => {
  const m = (manifest as Manifest)[v.slug] ?? { 'with-decor': [], 'without-decor': [] }
  return {
    slug: v.slug,
    name: v.name,
    tagline: v.tagline,
    ...
    galleryWithDecor: m['with-decor'].map((f) => `/images/venues/${v.slug}/with-decor/${f}`),
    galleryWithoutDecor: m['without-decor'].map((f) => `/images/venues/${v.slug}/without-decor/${f}`),
  }
})
