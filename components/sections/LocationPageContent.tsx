  ...
  listItem: {
    bullet: ({ children }) => (
      <li style={{ display: 'flex', alignItems: 'flex-start', gap: '0.6rem' }}>
        <span style={{ flexShrink: 0, marginTop: '0.55em', width: '4px', height: '4px', background: 'var(--color-gold)' }} />
...
export async function LocationPageContent({ page }: { page: LocationPageDoc }) {
  ...
  return (
    <main>
      <section data-nav-surface="dark" style={{ background: 'var(--color-ink)', padding: 'clamp(6rem, 14vh, 9rem) clamp(1.25rem, 5vw, 3rem) clamp(3rem, 7vh, 4.5rem)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
              {localities.map((l) => (
                <Link key={l.slug} href={`/${l.slug}/`} style={{
                  ...
                  borderTop: '2px solid var(--color-gold)', padding: '1.5rem', textDecoration: 'none',
                }}>
                  <span style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: '0.68rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--color-gold)' }}>
